...
from agent.solution_search import SFT_NALP_LLMSolutionSearchAgent
from common.utils import remove_comments, normalize_spaces

SOLVER_AGENT_DICT = {
    'sft_vanilla': SFT_NALP_LLMSolutionSearchAgent
}
...
def main(
    ...
    logger.info(f'Evaluating H-BFS on {benchmark} with hyperparams: {saved_args}')
            ...
            while n_cost_attempts < solver_max_search_trials:
                try:
                    ...
                    result = await solution_searcher.search_async(
                        ...
                        init_solution_state,
                        tag=f'{tag_i}-{try_i}/{try_num}',
                        early_exit_with_submission=True,
                        verbose=verbose,
                    )
                    n_cost_attempts += result.cost
                    results.append(result)
                    if any(result.search_graph.nodes(data=True)[n]['rpe_proof'] is not None for n in result.final_nodes) > 0: # Success@Budget
