...
def main(
    ...
    log_prefix = benchmark+'.'+'h_sa'+'.'

    os.makedirs(log_root, exist_ok=True)
    logger.remove()
    logger.add(sys.stdout, level=('INFO' if not verbose else 'DEBUG'), filter=(lambda record: record["name"] != "agent.solution_autoformalization" if not verbose else (lambda : True)))
    logger.add(osp.join(log_root, log_prefix+now+'.log'), level='DEBUG')
    logger.info(f'Evaluating H-SA on {benchmark} with hyperparams: {saved_args}')
    ...
    async def search(sample: FormalProblem, tag_i: int) -> None:
        ...
        await solving_server.load_problem_async(sample)
        for try_i in range(try_num):
            try:
                time_start = time.time()

                solution_autoformalizer = SolutionAutoformalizer(
                    ...
                    autoformalization_client=solver_client,
                    autoformalization_model=solver_model_name,
                    demonstrations=None,
                    try_num=try_num,
                    temperature=temperature,
                    dynamic_temperatre=False,
                    max_tokens=(max_tokens if max_tokens > 0 else NOT_GIVEN),
                )
                log_debug(f"search({tag_i}-{try_i}/{try_num}): server initialized.")
                result = await solution_autoformalizer.autoformalize_async(solving_server.sample, str(tag_i))
                results.append(result)
                if result.success:
                    logger.opt(colors=True).info(f'<green>search({tag_i}): succeeded at the {try_i}-th attempt in {time.time() - time_start} (s).</green>')
                else:
                    logger.info(f'search({tag_i}): failed at the {try_i}-th attempt in {time.time() - time_start} (s).')
            except:
